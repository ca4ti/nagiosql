...
$chkTfName          		= isset($_POST['tfName'])         			? str_replace(" ","_",$_POST['tfName']) : "";
...
// ============================
if (!isset($_SESSION['search']) || !isset($_SESSION['search']['service'])) 		$_SESSION['search']['service'] 		= "";
if (!isset($_SESSION['search']) || !isset($_SESSION['search']['serviceorder'])) $_SESSION['search']['serviceorder'] = "";
if (($chkModus == "checkform") || ($chkModus == "filter")) {
  	$_SESSION['search']['service'] 		= $chkTfSearch;
  	$_SESSION['search']['serviceorder'] = $chkSelOrderBy;
  	if ($chkSelOrderByGet != "") {
    	$_SESSION['search']['serviceorder'] = $chkSelOrderByGet;
  	}
  	if ($chkSelOrderBy == translate('All configs')) {
    	$_SESSION['search']['serviceorder'] = "";
  	}	
}
//$strFilter = "";
//
...
if ($chkNOnull == "") {$strNO = substr($chkNOw.$chkNOu.$chkNOc.$chkNOr.$chkNOf.$chkNOs,0,-1);} else {$strNO = "null";}
...
if (($chkModus == "insert") || ($chkModus == "modify")) {
	...
	if (($chkTfName != "") && ($chkTfServiceDescription != "")) {
    	...
    	if ($intInsert == 1) {
      		...
      		if ($chkModus == "insert") $myDataClass->writeLog(translate('New service inserted:')." ".$chkTfName);
      		if ($chkModus == "modify") $myDataClass->writeLog(translate('Service modified:')." ".$chkTfName);
      		//
      		// Insert/update relations
      		// =======================
      		if ($chkModus == "insert") {
        		if ($intSelHosts         != 0)  $myDataClass->dataInsertRelation("tbl_lnkServiceToHost",$chkDataId,$chkSelHosts);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
        		if ($intSelHostGroups    != 0)  $myDataClass->dataInsertRelation("tbl_lnkServiceToHostgroup",$chkDataId,$chkSelHostGroups);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
        		if ($intSelServiceGroups != 0)  $myDataClass->dataInsertRelation("tbl_lnkServiceToServicegroup",$chkDataId,$chkSelServiceGroups);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
        		if ($intSelContacts    	 != 0)  $myDataClass->dataInsertRelation("tbl_lnkServiceToContact",$chkDataId,$chkSelContacts);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
        		if ($intSelContactGroups != 0)  $myDataClass->dataInsertRelation("tbl_lnkServiceToContactgroup",$chkDataId,$chkSelContactGroups);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
			...
			// ===========================================================
     		 if (($chkModus == "modify") && ($chkOldConfig != $chkTfName) && ($chkDomainId != 0)) {
        		$intServiceCount = $myDBClass->countRows("SELECT * FROM `tbl_service` WHERE BINARY `config_name`='$chkOldConfig' AND `config_id`=$chkDomainId AND `active`='1'");
        		if ($intServiceCount == 0) {
					...
					$myVisClass->processMessage($myConfigClass->strDBMessage,$strMessage);
          			if ($intReturn == 0) {
						$myVisClass->processMessage(translate('The assigned, no longer used configuration files were deleted successfully!')."<br>",$strMessage);
            			$myDataClass->writeLog(translate('Service file deleted:')." ".$chkOldConfig.".cfg");
          			} else {
						if ($chkDomainId == 0) {
							$myVisClass->processMessage(translate('Common files cannot be removed from target systems - please check manually'),$strMessage);
						} else {
							$myVisClass->processMessage(translate('Errors while deleting the old configuration file - please check!:')."<br>".$myConfigClass->strDBMessage."<br>",$strMessage);
						}
          			}
        		}
      		}
      		...
      		if (($chkModus == "modify") && ($chkActive == 0)) {
        		...
        		if ($intServiceCount == 0) {
					...
					$myVisClass->processMessage($myConfigClass->strDBMessage,$strMessage);
          			if ($intReturn == 0) {
						$myVisClass->processMessage(translate('The assigned, no longer used configuration files were deleted successfully!')."<br>",$strMessage);
            			$myDataClass->writeLog(translate('Service file deleted:')." ".$chkTfName.".cfg");
          			} else {
						$myVisClass->processMessage(translate('Errors while deleting the old configuration file - please check!:')."<br>".$myConfigClass->strDBMessage."<br>",$strMessage);
          			}
        		}
			...
			*/
			...
			// Update Import HASH
			// ==================
			$booReturn = $myDataClass->updateHash('tbl_service',$chkDataId);
			$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
      		$intReturn = 0;
    	}
  	} else {
	...
	if ($setEnableCommon != 0) {
  	...
  	$strSQL  = "SELECT `id`, `config_name` FROM `tbl_service` WHERE $strDomainWhere GROUP BY `config_name`";
  	$myDBClass->getDataArray($strSQL,$arrData,$intDataCount);
  	$intError = 0;
  	if ($intDataCount != 0) {
    	foreach ($arrData AS $data) {
      		$booReturn = $myConfigClass->createConfigSingle("tbl_service",$data['id']);
			$myVisClass->processMessage($myConfigClass->strDBMessage,$strMessage);
	...
	// Display additional relation information
  	$myDataClass->infoRelation("tbl_service",$chkListId,"config_name,service_description");
  	$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
  	$intReturn   = 0;
  	$chkModus    = "display";
	...
	// Copy selected datasets
  	$intReturn   = $myDataClass->dataCopyEasy("tbl_service","config_name",$chkListId,$chkSelTargetDomain);
  	$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
  	$chkModus    = "display";
} else if (($chkModus == "checkform") && ($chkSelModify == "activate")) {
	// Activate selected datasets
	$intReturn   = $myDataClass->dataActivate("tbl_service",$chkListId);
  	$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
	$chkModus    = "display";
} else if (($chkModus == "checkform") && ($chkSelModify == "deactivate")) {
	// Deactivate selected datasets
	$intReturn   = $myDataClass->dataDeactivate("tbl_service",$chkListId);
  	$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
	$chkModus    = "display"; 
	...
	// Write configuration file
  	...
  	// Check the active status of the configuration
  	$booReturn = $myDBClass->getSingleDataset("SELECT `config_name`, `active` FROM `tbl_service` WHERE `id`=".$intDSId,$arrModifyData);
  	$myDBClass->getSingleDataset("SELECT count(*) AS `counter` FROM `tbl_service` WHERE `config_name`='".$arrModifyData['config_name']."' AND `config_id`=$chkDomainId",$arrActiveData);
	$myVisClass->processMessage($myDBClass->strDBError,$strMessage);
  	// Backup config file if no active configuration exists
...
if ($chkModus == "add") {
	// Process template fields
  	...
  	$strSQL   	= "SELECT `id`,`template_name` FROM `tbl_servicetemplate` WHERE $strDomainWhere ORDER BY `template_name`";
  	...
  	$strSQL     = "SELECT `id`, `name` FROM `tbl_service` WHERE `name` <> '' AND $strDomainWhere $strWhere ORDER BY `name`";
	...
	$intReturn2 = $myVisClass->parseSelectMulti('tbl_hostgroup','hostgroup_name','hostgroup','tbl_lnkServiceToHostgroup',2,$intFieldId);
  	if (($intReturn1 != 0) && ($intReturn2 != 0)) $strDBWarning .= translate('Attention, no hosts or hostgroups defined!')."<br>";
	...
	if ($intReturn != 0) $strDBWarning .= translate('Attention, no check commands defined!')."<br>";
	...
	$conttp->setVariable("ORDER_BY",$_SESSION['search']['serviceorder']);
...
if ($chkModus == "display") {
  	...
  	$mastertp->setVariable("DAT_SEARCH",$_SESSION['search']['service']);
	...
	$myVisClass->insertDomainList($mastertp);
  	...
  	$strSQL    = "SELECT DISTINCT `config_name` FROM `tbl_service` WHERE $strDomainWhere ORDER BY `config_name`";
  	...
  	if ($booReturn == false) {
		...
		for ($i=0;$i<$intDataCount;$i++) {
      		...
      		if ($_SESSION['search']['serviceorder'] == $arrDataConfig[$i]['config_name']) {
  	...
  	$strSearchWhere = "";
  	if ($_SESSION['search']['service'] != "") {
		$strSearchTxt   = $_SESSION['search']['service'];
    	$strSearchWhere = "AND (`config_name` LIKE '%".$strSearchTxt."%' OR `service_description` LIKE '%".$strSearchTxt."%' OR
                		   `display_name` LIKE '%".$strSearchTxt."%')";
  	}
	$strFilter = "";
  	if ($_SESSION['search']['serviceorder'] != "") {
		$strSearchTxt   = $_SESSION['search']['serviceorder'];
    	$strFilter 		= " AND `config_name` = '".$strSearchTxt."' ";
  	}
  	// Count datasets
  	$strSQL    = "SELECT count(*) AS `number` FROM `tbl_service` WHERE `config_id`=$chkDomainId $strFilter $strSearchWhere AND `access_group` IN ($strAccess)";
	...
	if ($intCount == 0) {
  	...
  	$strSQL    = "SELECT `id`, `config_name`, `service_description`, `active`, `last_modified`, `config_id` FROM `tbl_service` WHERE $strDomainWhere
         		  $strFilter $strSearchWhere AND `access_group` IN ($strAccess) ORDER BY `config_id`,`config_name`,`service_description` 
  	...
  	if ($booReturn == false) {
    	...
    	for ($i=0;$i<$intDataCount;$i++) {
			...
			// Set datafields
	  		...
	  		$mastertp->setVariable("DATA_FIELD_1S",addslashes(htmlspecialchars($arrDataLines[$i]['config_name'],ENT_COMPAT,'UTF-8')));
      		$mastertp->setVariable("DATA_FIELD_2",htmlspecialchars($arrDataLines[$i]['service_description'],ENT_COMPAT,'UTF-8'));
	  		$mastertp->setVariable("DATA_FIELD_2S",addslashes(htmlspecialchars($arrDataLines[$i]['service_description'],ENT_COMPAT,'UTF-8')));
	...
	// Show page numbers
  	$mastertp->setVariable("IMAGE_PATH",$SETS['path']['root']."images/");
  	if (isset($intCount)) $mastertp->setVariable("PAGES",$myVisClass->buildPageLinks(filter_var($_SERVER['PHP_SELF'], FILTER_SANITIZE_STRING),$intCount,$chkLimit,$_SESSION['search']['serviceorder']));
