...
// ============================
if (!isset($_SESSION['search']) || !isset($_SESSION['search']['contacttemplate'])) $_SESSION['search']['contacttemplate'] = "";
if (($chkModus == "checkform") || ($chkModus == "filter")) {
  $_SESSION['search']['contacttemplate'] = $chkTfSearch;
}
//
...
if (($chkModus == "insert") || ($chkModus == "modify")) {
	...
	$strSQLx = "`tbl_contacttemplate` SET `template_name`='$chkTfName', `alias`='$chkTfFriendly', `contactgroups`=$intContactGroups,
				...
				`use_variables`='$intVariables', `use_template`=$intTemplates, `use_template_tploptions`=$chkRadTemplates, 
				 $strGroupSQL $strDomain `active`='$chkActive', `config_id`=$chkDomainId, `last_modified`=NOW()";
	if ($chkModus == "insert") {
	...
	if ($chkTfName != "") {	
		...
		$myDataClass->updateStatusTable("tbl_contacttemplate");
		if ($chkModus == "insert") $chkDataId = $intInsertId;
		if ($intInsert == 1) {
			$intReturn = 1;
		} else {
			if ($chkModus  == "insert") $myDataClass->writeLog(translate('New contact template inserted:')." ".$chkTfName);
			if ($chkModus  == "modify") $myDataClass->writeLog(translate('Contact template modified:')." ".$chkTfName);
      		//
      		// Insert/update relations
      		// =======================
			if ($chkModus == "insert") {
				if ($intContactGroups  != 0) $myDataClass->dataInsertRelation("tbl_lnkContacttemplateToContactgroup",$chkDataId,$chkSelContactGroup);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
				if ($intHostCommand    != 0) $myDataClass->dataInsertRelation("tbl_lnkContacttemplateToCommandHost",$chkDataId,$chkSelHostCommand);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
				if ($intServiceCommand != 0) $myDataClass->dataInsertRelation("tbl_lnkContacttemplateToCommandService",$chkDataId,$chkSelServiceCommand);
				$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
	...
	$myDataClass->infoRelation("tbl_contacttemplate",$chkListId,"template_name");
	$myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
	$intReturn   = 0;
	$chkModus    = "display";
	...
	$intReturn   = $myDataClass->dataCopyEasy("tbl_contacttemplate","template_name",$chkListId,$chkSelTargetDomain);
    $myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
	$chkModus    = "display";
} else if (($chkModus == "checkform") && ($chkSelModify == "activate")) {
  	// Activate selected datasets
  	$intReturn   = $myDataClass->dataActivate("tbl_contacttemplate",$chkListId);
    $myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
  	$chkModus    = "display";
} else if (($chkModus == "checkform") && ($chkSelModify == "deactivate")) {
  	// Deactivate selected datasets
  	$intReturn   = $myDataClass->dataDeactivate("tbl_contacttemplate",$chkListId);
    $myVisClass->processMessage($myDataClass->strDBMessage,$strMessage);
  	$chkModus    = "display";
...
if ($chkModus == "add") {
	...
	$strSQL 	= "SELECT `id`,`template_name` FROM `tbl_contacttemplate` WHERE `config_id`=$chkDomainId $strWhere ORDER BY template_name";
	...
	$strSQL 	= "SELECT `id`, `name` FROM `tbl_contact` WHERE `name` <> '' AND `config_id`=$chkDomainId ORDER BY name";
...
if ($chkModus == "display") {
	...
	$mastertp->setVariable("DAT_SEARCH",$_SESSION['search']['contacttemplate']);
	...
	$strSearchWhere = "";
	if ($_SESSION['search']['contacttemplate'] != "") {
		$strSearchTxt   = $_SESSION['search']['contacttemplate'];
		$strSearchWhere = "AND (`template_name` LIKE '%".$strSearchTxt."%' OR `alias` LIKE '%".$strSearchTxt."%' OR
	...
	// Count datasets
	$strSQL    = "SELECT count(*) AS `number` FROM `tbl_contacttemplate` WHERE $strDomainWhere $strSearchWhere AND `access_group` IN ($strAccess)";
	...
	// Get datasets
	$strSQL    = "SELECT `id`, `template_name`, `alias`, `active`, `config_id` FROM `tbl_contacttemplate` WHERE $strDomainWhere $strSearchWhere
