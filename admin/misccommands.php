...
if (($chkModus == "insert") || ($chkModus == "modify")) {
	...
	$strSQL2 = "tbl_misccommand SET command_name='$chkInsName', command_line='$chkInsCommand', active='$chkActive', last_modified=NOW()";
	if ($chkModus == "insert") {
		$strSQL1 = "INSERT INTO "; 
		$strSQL3 = "";
	} else {
	...
	if (($chkInsName != "") && ($chkInsCommand != "")) {
		...
		if ($chkModus == "insert") $myVisClass->writeLog($LANG['logbook']['newmisccmd']." ".$chkInsName);
		if ($chkModus == "modify") $myVisClass->writeLog($LANG['logbook']['modifymisccmd']." ".$chkInsName);
	} else {
	...
	$myVisClass->createConfig("tbl_misccommand");
	$strMessage = $myVisClass->strDBMessage;
	$chkModus = "display"; 
}  else if (($chkModus == "checkform") && ($chkSelModify == "delete")) {
	// Gewählte Datensätze löschen
	$myVisClass->dataDelete("tbl_misccommand",$chkListId);
	$strMessage = $myVisClass->strDBMessage;
	$chkModus = "display";
} else if (($chkModus == "checkform") && ($chkSelModify == "copy")) {
	// Gewählte Datensätze kopieren
	$myVisClass->dataCopy("tbl_misccommand",$chkListId);
	$strMessage = $myVisClass->strDBMessage;
	$chkModus = "display";
...
// ======================================
$myVisClass->lastModified("tbl_misccommand",$strLastModified,$strFileDate,$strOld);
//
// HTML Template laden
// ===================
...
if ($chkModus == "display") {
	...
	// Datensätze holen
	$strSQL    = "SELECT id, command_name, command_line, active FROM tbl_misccommand ORDER BY command_name LIMIT $chkLimit,15";
...
$strContMessage = $myVisClass->checkConsistMisccommands();
$mastertp->setVariable("CONSISTUSAGE",$strContMessage);
if ($strContMessage == $LANG['admincontent']['misccommandsok']) {
